                        ...
                        <div class="sidebar__item">   
                            ...
                            <ul>
                                <li <c:if test="${PSUBCATEGORY eq null}"> class="active" </c:if>>
                                	<a href="list.product?PCATEGORY=식품">ALL</a></li>
                                	
                                <li <c:if test="${PSUBCATEGORY eq '과일'}"> class="active" </c:if>>
                                	<a href="list.product?PCATEGORY=식품&PSUBCATEGORY=과일">과일</a></li>
                                	
                                <li <c:if test="${PSUBCATEGORY eq '채소'}"> class="active" </c:if>>
                                	<a href="list.product?PCATEGORY=식품&PSUBCATEGORY=채소">채소</a></li>
                                	
                                <li <c:if test="${PSUBCATEGORY eq '육류∙계란'}"> class="active" </c:if>>
                                	<a href="list.product?PCATEGORY=식품&PSUBCATEGORY=육류∙계란">육류 ∙ 계란</a></li>
                                
                                <li <c:if test="${PSUBCATEGORY eq '해산물'}"> class="active" </c:if>>
                                	<a href="list.product?PCATEGORY=식품&PSUBCATEGORY=해산물">해산물</a></li>
                                
                                <li <c:if test="${PSUBCATEGORY eq '간편식∙밀키트'}"> class="active" </c:if>>
                                	<a href="list.product?PCATEGORY=식품&PSUBCATEGORY=간편식∙밀키트">간편식 ∙ 밀키트</a></li>
                            </ul>
                        </div>
                            ...
                            %>
                            	<div class="sidebar__item__price">
	                                <input type="button" name="lineUpPrice" value="전체 선택" 
	                                	...
	                                	location.href='list.product?PCATEGORY=식품&PSUBCATEGORY=${PSUBCATEGORY}'">
	                            </div>
                            ...
                            <c:forEach var="i" begin="0" end="${fn:length(lineUpPrice_arr)-1}" step="1">
	                            <div class="sidebar__item__price">
	                                <input type="button" name="lineUpPrice" value="${lineUpPrice_arr[i]}" 
	                                	...
	                                	onClick="if(this.value) 
	                                	location.href='list.product?&lineUpPrice=${lineUpPrice_arr[i]}&PCATEGORY=식품&PSUBCATEGORY=${PSUBCATEGORY}'">
	                            </div>
                            </c:forEach>
                            ...
                            <div class="col-lg-4 col-md-5">
                                <div class="filter__sort">
                                    ...
                                    <select onChange="if(this.value) location.href=(this.value);">
                                        <option value="list.product?PCATEGORY=식품&PSUBCATEGORY=${PSUBCATEGORY}"
                                        	<c:if test="${fn:contains(lineUp, null)}"> selected </c:if> >추천순</option>
                                        <option value="list.product?PCATEGORY=식품&PSUBCATEGORY=${PSUBCATEGORY}&lineUpPrice=${lineUpPrice}&lineUp=신상품순"
                                        	<c:if test="${fn:contains(lineUp, '신상품순')}"> selected </c:if> >신상품순</option>
                                        <option value="list.product?PCATEGORY=식품&PSUBCATEGORY=${PSUBCATEGORY}&lineUpPrice=${lineUpPrice}&lineUp=인기상품순"
                                        	<c:if test="${fn:contains(lineUp, '인기상품순')}"> selected </c:if> >인기상품순</option>
                                        <option value="list.product?PCATEGORY=식품&PSUBCATEGORY=${PSUBCATEGORY}&lineUpPrice=${lineUpPrice}&lineUp=낮은가격순"
                                        	<c:if test="${fn:contains(lineUp, '낮은가격순')}"> selected </c:if> >낮은 가격순</option>
                                        <option value="list.product?PCATEGORY=식품&PSUBCATEGORY=${PSUBCATEGORY}&lineUpPrice=${lineUpPrice}&lineUp=높은가격순"	
