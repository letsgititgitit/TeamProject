...
<script type="text/javascript">
    ...
    <section class="breadcrumb-section set-bg" data-setbg="img/banner_pet.jpg">
        <div class="container">
            <div class="row">
                        ...
                        <div class="sidebar__item">   
                            ...
                            <ul>
                                <li <c:if test="${PSUBCATEGORY eq null}"> class="active" </c:if>>
                                	<a href="list.product?PCATEGORY=반려동물">ALL</a></li>
                                	
                                <li <c:if test="${PSUBCATEGORY eq '사료'}"> class="active" </c:if>>
                                	<a href="list.product?PCATEGORY=반려동물&PSUBCATEGORY=사료">사료</a></li>
                                	
                                <li <c:if test="${PSUBCATEGORY eq '간식'}"> class="active" </c:if>>
                                	<a href="list.product?PCATEGORY=반려동물&PSUBCATEGORY=간식">간식</a></li>
                                	
                                <li <c:if test="${PSUBCATEGORY eq '용품'}"> class="active" </c:if>>
                                	<a href="list.product?PCATEGORY=반려동물&PSUBCATEGORY=용품">용품</a></li>
                            </ul>
                        </div>
                            ...
                            %>
                            	<div class="sidebar__item__price">
	                                <input type="button" name="lineUpPrice" value="전체 선택" 
	                                	...
	                                	location.href='list.product?PCATEGORY=반려동물&PSUBCATEGORY=${PSUBCATEGORY}'">
	                            </div>
                            ...
                            <c:forEach var="i" begin="0" end="${fn:length(lineUpPrice_arr)-1}" step="1">
	                            <div class="sidebar__item__price">
	                                <input type="button" name="lineUpPrice" value="${lineUpPrice_arr[i]}" 
	                                	...
	                                	onClick="if(this.value) 
	                                	location.href='list.product?&lineUpPrice=${lineUpPrice_arr[i]}&PCATEGORY=반려동물&PSUBCATEGORY=${PSUBCATEGORY}'">
	                            </div>
                            </c:forEach>
                            ...
                            <div class="col-lg-4 col-md-5">
                                <div class="filter__sort">
                                    ...
                                    <select onChange="if(this.value) location.href=(this.value);">
                                        <option value="list.product?PCATEGORY=반려동물&PSUBCATEGORY=${PSUBCATEGORY}"
                                        	<c:if test="${fn:contains(lineUp, null)}"> selected </c:if> >추천순</option>
                                        <option value="list.product?PCATEGORY=반려동물&PSUBCATEGORY=${PSUBCATEGORY}&lineUpPrice=${lineUpPrice}&lineUp=신상품순"
                                        	<c:if test="${fn:contains(lineUp, '신상품순')}"> selected </c:if> >신상품순</option>
                                        <option value="list.product?PCATEGORY=반려동물&PSUBCATEGORY=${PSUBCATEGORY}&lineUpPrice=${lineUpPrice}&lineUp=인기상품순"
                                        	<c:if test="${fn:contains(lineUp, '인기상품순')}"> selected </c:if> >인기상품순</option>
                                        <option value="list.product?PCATEGORY=반려동물&PSUBCATEGORY=${PSUBCATEGORY}&lineUpPrice=${lineUpPrice}&lineUp=낮은가격순"
                                        	<c:if test="${fn:contains(lineUp, '낮은가격순')}"> selected </c:if> >낮은 가격순</option>
                                        <option value="list.product?PCATEGORY=반려동물&PSUBCATEGORY=${PSUBCATEGORY}&lineUpPrice=${lineUpPrice}&lineUp=높은가격순"	
